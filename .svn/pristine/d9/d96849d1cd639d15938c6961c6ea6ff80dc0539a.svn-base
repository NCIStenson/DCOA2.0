...
- (void)layoutOfficalDocsubViews {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        tableView.mj_header = header;        [tableViewArr addObject:tableView];
...
-(void)segmentedConValueChanged:(CCitySegmentedControl *)segCon {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
          ...
          [_scrollView setContentOffset:CGPointMake(self.view.bounds.size.width*segCon.selectedIndex, 0) animated:YES];
    });
...
- (void)configDataWithURL:(NSString*)url {
   ...
   currentTask = [manager GET:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        cell = [[CCityOfficalBackLogCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:officalBackLogCellReuseId conentMode:contentMode];
    }

...
- (void) okAction {
    ...
    NSString* str = [NSDateFormatter localizedStringFromDate:_datePicker.date dateStyle:NSDateFormatterShortStyle timeStyle:NSDateFormatterNoStyle];
    NSArray* times = [str componentsSeparatedByString:@"/"];
    str = [NSString stringWithFormat:@"%@-%@-%@",times[0],times[1],times[2]];
    
    if (self.slelectAction) {   self.slelectAction(str);    }

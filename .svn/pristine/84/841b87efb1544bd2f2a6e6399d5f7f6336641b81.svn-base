...
-(NSInteger)addDatasWithArr:(NSArray*)result {
    ...
    for (int i = 0; i < result.count; i++) {
        ...
        if (self.datasMuArr.count) {
            ...
            for (int j = 0; j < self.datasMuArr.count; j++) {
                
                CCityMainDocSearchModel* inModel = self.datasMuArr[j];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell.accessoryCon) {
        [cell.accessoryCon addTarget:self action:@selector(accessorySelected:) forControlEvents:UIControlEventTouchUpInside];
    }
    return cell;
}
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    UIImageView* imageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ccity_judgement_50x50_"]];
    ...
    bottomLine.frame = CGRectMake(0, 43.5, self.view.bounds.size.width, .5f);
    ...
    [imageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(sectionHeader).with.offset(5.f);
        make.left.equalTo(sectionHeader).with.offset(10.f);
        make.bottom.equalTo(sectionHeader).with.offset(-5.f);

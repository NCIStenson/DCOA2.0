...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ((NSInteger)model.mainStyle == -1) {
        ...
        if ([model.messageType isEqualToString:@"会议消息"]) {
            ...
            CCityMeetingDeitalVC* meetingVC = [[CCityMeetingDeitalVC alloc]initWithUrl:@"service/message/GetMeetingFromMessage.ashx" parameters:@{@"token":[CCitySingleton sharedInstance].token, @"messageId":model.messageId}];

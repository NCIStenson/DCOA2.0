...
#define CCITY_BEGIN_TIME @"2015-01-01"
#define CCITY_END_TIME   @"2016-02-23"
...
- (void)viewDidLoad {
    ...
    self.title = @"高级搜索";
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithCustomView:[self backControl]];
...
//    [parameters setObject:@(_contentStyle) forKey:@"metatype"];
    ...
    for (int i = 0; i < _datasArr.count; i++) {
        ...
        CCityMainDetailSearchModel* model = _datasArr[i];
    ...
    [manager POST:_url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (!results.count) {
            ...
            CCityMainTaskSearchResultVC* resultVC = [[CCityMainTaskSearchResultVC alloc]initDatas:datas url:_url parameters:parameters];
...
- (void)configData {
    ...
    if (_contentStyle == CCithMainDetailTaskSearchContentDocStyle) {
        ...
        _datasArr = [NSMutableArray arrayWithCapacity:3];

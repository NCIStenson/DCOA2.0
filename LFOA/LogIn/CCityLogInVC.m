...
- (UIView*)logo {
    ...
    UIImageView* logImage = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ccity_login_logo_150x150"]];
...
-(void)logInSuccessWith:(NSDictionary*)response {
    ...
    [CCitySingleton sharedInstance].phoneNum = response[@"mobilePhone"];
    [CCitySingleton sharedInstance].organization = response[@"organization"];
    if (response[@"privilege"] != [NSNull null]) {
        [CCitySingleton sharedInstance].jurisdictionArr = response[@"privilege"];
    }
    ...
    if ([[UIApplication sharedApplication] keyWindow].rootViewController == self) {
        ...
        if (_notificDic) {  [CCityNoficManager reciveNotficWithDic:_notificDic];    }
    } else {
